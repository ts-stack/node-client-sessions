...
var vows = require("vows"),
    ...
    cookieSessions = require("../dist/client-session-factory").clientSessionFactory,
...
suite.addBatch({
  "public encode and decode util methods" : {
    ...
    "encode " : function(err, req){
      var result = util.encode({cookieName: 'session', secret: 'yo'}, {foo:'bar'});
      var result_arr = result.split(".");
      assert.equal(result_arr.length, 5);
    },
    "encode and decode - is object" : function(err, req){
      var encoded = util.encode({cookieName: 'session', secret: 'yo'}, {foo:'bar'});
      var decoded = util.decode({cookieName: 'session', secret: 'yo'}, encoded);
      assert.isObject(decoded);
    },
    "encode and decode - has all values" : function(err, req){
      var encoded = util.encode({cookieName: 'session', secret: 'yo'}, {foo:'bar', bar:'foo'});
      var decoded = util.decode({cookieName: 'session', secret: 'yo'}, encoded);
    ...
    "encode and decode - override duration and createdAt" : function(err, req){
      var encoded = util.encode({cookieName: 'session', secret: 'yo'}, {foo:'bar', bar:'foo'}, 5000, 1355408039221);
      var decoded = util.decode({cookieName: 'session', secret: 'yo'}, encoded);
    ...
    "encode and decode - tampered HMAC" : function(err, req){
      ...
      var decodedReal = util.decode({cookieName: 'session', secret: 'yo'}, encodedReal);
      assert.isObject(decodedReal);
      var decodedFake = util.decode({cookieName: 'session', secret: 'yo'}, encodedFake);
    ...
    "decode - invalid input" : function(err, req){
        ...
        assert.isUndefined(util.decode({cookieName: 'session', secret: 'yo'}, notEnoughComponents));
        ...
        assert.isUndefined(util.decode({cookieName: 'session', secret: 'yo'}, invalidBase64));
    }
  }
});
